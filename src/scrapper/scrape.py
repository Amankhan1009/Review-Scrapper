...
#             values = [[data.iloc[i, j] for j in range(len(data.columns))] for i in range(len(data))]
...
class ScrapeReviews:
    ...
    def extract_reviews(self, product_link):
        try:
            productLink = "https://www.myntra.com/" + product_link
            self.current_product_link = productLink  # Save it for later
            self.driver.get(productLink)
            prodRes = self.driver.page_source
            prodRes_html = bs(prodRes, "html.parser")
            title_h = prodRes_html.findAll("title")
        ...
        try:
            ...
            for i in range(len(user_rating)):
                ...
                mydict = {
                    ...
                    "Name": name,
                    "Comment": comment,
                    "Product Link": self.current_product_link
                }
